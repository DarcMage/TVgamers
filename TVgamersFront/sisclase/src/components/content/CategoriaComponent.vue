        ...
        <div class="row">
			<div class="col-md-12 col-sm-12 ">
				<div class="x_panel" id="tabladatosmenus" v-show="!mostrarformulario">
					...
					<tbody>
						<tr v-for="itemCategoria in listacategoria" v-bind:key="itemCategoria" v-bind:itemCategoria="itemCategoria" >
						  ...
						  <td class="text-center">
							...
							<i class="fa fa-trash" v-on:click="eliminarCategoria(itemCategoria.codigo)"  style="cursor:pointer; color:red"></i>
						  </td>
                        </tr>   
				...
				<div class="x_panel" id="formulariomenus" v-show="mostrarformulario">
					...
					<div class="x_content">
						<br />
						<form id="frmCategoria" class="form-horizontal form-label-left">
							<input type="hidden" id="id" name="id" v-model="id"/>
							<div class="item form-group">
								<label class="col-form-label col-md-3 col-sm-3 label-align" for="nombre">Nombre de Categoria <span class="required"></span>
								</label>
...
export default{
	...
	methods: {
		async iniciarCarga() {
			try {
			const response = await fetch("http://localhost:8080/v1/categoria/");
			this.listacategoria= this.listCategoriaItems = await response.json();   
			} catch (error) {
			//console.log(error);
			}
		},
		...
		async eliminarCategoria(codigotmp){
			try {
			const response =await fetch("http://localhost:8080/v1/categoria/"+codigotmp,{
				method: "DELETE",
			})
